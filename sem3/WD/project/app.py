...
app.layout = html.Div(
    [
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H1(
                            "NBA History & Evolution Dashboard",
        ...
        # Loading overlay
        dcc.Loading(
            id="loading-overlay", type="default", children=[html.Div(id="tab-content")]
        ),
        # Loading message
        html.Div(
            id="loading-message",
...
@app.callback(
    [Output("data-store", "data"), Output("loading-message", "style")],
                    ...
                    dbc.Col(
                        [
                            ...
                            dcc.Dropdown(
                                id="season-dropdown",
                                options=[
                                    {"label": s, "value": s} for s in available_seasons
                                ],
...
def update_radar(n_clicks, season_a, season_b, data):
    ...
    fig.update_layout(
        polar=dict(
            radialaxis=dict(visible=True, tickfont={"color": "white"}),
...
def update_shooting(n_clicks, season_a, season_b, data):
    ...
    season_a_fgpct = (
        (season_a_data["fieldGoalsMade"].sum() / season_a_fg_attempts * 100)
        if season_a_fg_attempts > 0
        else 0
    )
    season_a_3ppct = (
        (season_a_data["threePointersMade"].sum() / season_a_3p_attempts * 100)
        if season_a_3p_attempts > 0
        else 0
    )
    season_a_ftpct = (
        (season_a_data["freeThrowsMade"].sum() / season_a_ft_attempts * 100)
        if season_a_ft_attempts > 0
        else 0
    )
    ...
    season_b_fgpct = (
        (season_b_data["fieldGoalsMade"].sum() / season_b_fg_attempts * 100)
        if season_b_fg_attempts > 0
        else 0
    )
    season_b_3ppct = (
        (season_b_data["threePointersMade"].sum() / season_b_3p_attempts * 100)
        if season_b_3p_attempts > 0
        else 0
    )
    season_b_ftpct = (
        (season_b_data["freeThrowsMade"].sum() / season_b_ft_attempts * 100)
        if season_b_ft_attempts > 0
        else 0
    )
    ...
    fig.add_trace(
        go.Bar(
            name=season_a, x=shooting_stats, y=season_a_shooting, marker_color="#17408B"
        )
    )

    fig.add_trace(
        go.Bar(
            name=season_b, x=shooting_stats, y=season_b_shooting, marker_color="#C9082A"
        )
    )
